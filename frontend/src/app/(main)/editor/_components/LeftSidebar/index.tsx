...
// ─── LeftSidebar ─────────────────────────────────────────────────────────────
// 왼쪽 사이드바. 펼침(w-56) / 접힘(w-12) 두 가지 레이아웃 처리.
...
export default function LeftSidebar({ isOpen, onToggle }: LeftSidebarProps) {
  ...
  const saveBtnLabel = isSaving ? 'Saving…' : saveStatus === 'saved' ? 'Saved ✓' : 'Save Draft'
        ...
        {isOpen ? (
          <>
            <div className="flex items-center justify-between mb-2 px-1.5">
              <p className="text-[10px] font-semibold text-gray-400 tracking-widest">MY LEDGERS</p>
              <button
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {loadingLedgers ? (
                ...
                <div className="px-2 py-2 text-xs text-gray-400">가계부 없음</div>
              ) : (
                ledgers.map(ledger => (
                  <LedgerRow
                    key={ledger.led_id}
        ...
        <div className={`flex flex-col ${isOpen ? 'gap-2' : 'gap-1 items-center'}`}>
          {isOpen ? (
            <>
              ...
              <button disabled title="준비 중" className="flex items-center justify-center gap-2 w-full px-3 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-lg opacity-50 cursor-not-allowed">
                <ImportIcon />
                Import Data
              </button>
            </>
            ...
            <>
              ...
              <button disabled title="Import Data (준비 중)" className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 opacity-50 cursor-not-allowed">
                <ImportIcon />
              </button>
            </>
          )}
