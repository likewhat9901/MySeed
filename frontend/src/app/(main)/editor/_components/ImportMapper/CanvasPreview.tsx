  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-4 py-3 border-b border-gray-100 shrink-0">
        <p className="text-xs font-semibold text-gray-500 tracking-widest">CANVAS</p>
        <p className="text-[11px] text-gray-400 mt-0.5">
          {selectedId
            ? `${selectedMeta?.label ?? '위젯'} 선택됨 — 오른쪽에서 셀 선택 후 연결하세요`
            : '위젯 클릭으로 선택 · 휠 클릭 드래그: 이동 · 휠 스크롤: 줌'}
        </p>
      </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    outline:       selected ? '2px solid #16a34a' : mapped ? '2px solid #86efac' : 'none',
                    outlineOffset: '1px',
                    boxShadow:     widget.style.dropShadow && !selected ? '0 2px 6px rgba(0,0,0,0.08)' : 'none',
                    zIndex:        selected ? 10 : 1,
                  }}
                  className={`overflow-hidden text-left transition-[outline] ${
                    selected ? 'bg-green-50' : 'bg-white hover:brightness-95'
                  }`}
                >
                  <div className="flex flex-col items-center justify-center w-full h-full gap-0.5 pointer-events-none">
                    {Icon && (
                      <Icon
                        size={iconSize}
                        className={selected ? 'text-green-600' : mapped ? 'text-green-400' : 'text-gray-400'}
                      />
                    )}
                    <span
                      style={{ fontSize }}
                      className={`font-medium leading-tight text-center px-1 w-full truncate ${
                        selected ? 'text-green-800' : mapped ? 'text-green-600' : 'text-gray-500'
                      }`}
                    >
