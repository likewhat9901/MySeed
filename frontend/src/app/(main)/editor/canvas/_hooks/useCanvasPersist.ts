...
export function useCanvasPersist(widgetsRef: React.RefObject<WidgetItem[]>): UseCanvasPersistReturn {
  ...
  useEffect(() => {
    ...
    async function loadLedger() {
      ...
      if (!ledId) {
        ...
        ledId = ledger.led_id
        name = ledger.led_name
      } else {
        // ?led 있음 → 이름만 목록에서 찾기
        const ledgers = await getMyLedgers(user.id)
        name = ledgers.find(l => l.led_id === ledId)?.led_name ?? null
      }
  ...
  const saveCanvas = useCallback(async () => {
    ...
    if (success) { setLastSavedAt(new Date()); setIsDirty(false) }
  ...
  // ── 다른 이름으로 저장 — 새 ledger 생성 후 현재 위젯 복사, 새 led_id 반환 ──
  const saveAsNewLedger = useCallback(async (name: string): Promise<string | null> => {
    const user = await getCurrentUser()
    if (!user) return null

    const newLedId = await createLedger(user.id, name)
    if (!newLedId) return null

    await saveCanvasWidgets(newLedId, widgetsRef.current ?? [])
    return newLedId
  }, [widgetsRef])

  return { canvasId, ledgerName, saveStatus, lastSavedAt, saveCanvas, saveAsNewLedger, initialWidgets, isDirty, markDirty }
}
