"""DB `tb_card.column_list` 순서([날짜, 가맹점, 금액] 헤더명) 기준 명세 엑셀 파싱."""
...
def ordered_header_labels(card_row: ImportCardRow | dict[str, Any]) -> tuple[str, str, str]:
    """
    `column_list` 순서 고정:
    인덱스 0 → 날짜, 1 → 가맹점, 2 → 금액 (엑셀 헤더 셀 문자열).
    """
...
def import_card_to_column_map(card_row: ImportCardRow | dict[str, Any]) -> dict[str, str]:
    """`build_record_rows`용 매핑: 가맹점은 title."""
    h_date, h_merchant, h_amount = ordered_header_labels(card_row)
    return {
...
def normalize_card_statement_rows(
    ...
    *,
    ...
    """
    엑셀 바이트와 DB `tb_card` 행을 받아 `(rows, warnings, meta)` 반환.
    헤더는 `column_list[0]`(날짜), `[1]`(가맹점), `[2]`(금액).
    """
    warnings: list[str] = []
    ...
    h_date, h_merchant, h_amount = ordered_header_labels(card_row)
    cols_raw = coerce_column_list(card_row.get("column_list"))
    if len(cols_raw) < 3:
        warnings.append("column_list에 요소가 3개 미만입니다(순서: 날짜, 가맹점, 금액).")
    if not h_date:
    ...
    if ix_date is None and h_date:
        warnings.append(f"'{h_date}' 헤더를 시트에서 찾지 못했습니다.")
    if ix_mrch is None and h_merchant:
        warnings.append(f"'{h_merchant}' 헤더를 시트에서 찾지 못했습니다.")
    if ix_amt is None and h_amount:
        warnings.append(f"'{h_amount}' 헤더를 시트에서 찾지 못했습니다.")
...
def _meta_bundle(
    ...
    return {
        ...
        "column_list": coerce_column_list(card_row.get("column_list")),
    }
